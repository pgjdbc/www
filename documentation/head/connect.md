...
of additional properties which can be used to specify additional driver behavior
...
* `compatible = String`
	...
	applications. At the moment this controls two driver behaviors: the
...
* `charSet = String`
	...
	The character set to use for data sent to the database or recieved from
	the database. This property is only relevent for server versions less
...
* `allowEncodingChanges = boolean`
	...
	one legitimate exception to this behavior though, using the `COPY` command
